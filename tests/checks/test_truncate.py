...
def test_truncate_single_table():
    ...
    # Arrange & Act & Assert
    assert check_detects_issue(TruncateCheck, "TRUNCATE TABLE audit_logs;", SeverityLevel.HIGH)
...
def test_truncate_multiple_tables():
    ...
    assert check_detects_issue(
        TruncateCheck, "TRUNCATE TABLE audit_logs, temp_records;", SeverityLevel.HIGH
    )
...
def test_truncate_without_table_keyword():
    ...
    # Arrange & Act & Assert
    assert check_detects_issue(TruncateCheck, "TRUNCATE audit_logs;", SeverityLevel.HIGH)
...
def test_truncate_with_cascade():
    ...
    # Arrange & Act & Assert
    assert check_detects_issue(
        TruncateCheck, "TRUNCATE TABLE audit_logs CASCADE;", SeverityLevel.HIGH
    )
...
def test_truncate_with_restart_identity():
    ...
    # Arrange & Act & Assert
    assert check_detects_issue(
        TruncateCheck, "TRUNCATE TABLE audit_logs RESTART IDENTITY;", SeverityLevel.HIGH
    )
...
def test_non_truncate_statement():
    ...
    # Arrange & Act & Assert
    assert check_no_issue(TruncateCheck, "DELETE FROM audit_logs WHERE created_at < '2023-01-01';")
